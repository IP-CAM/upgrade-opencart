...
class ModelUpgrade extends Model {
	public function mysql() {
		...
		// Drop table API
		$this->db->query("DROP TABLE `" . DB_PREFIX . "api`");

		// Create Table API
		$this->db->query($statements[5]);
		$this->db->query($statements[6]);
		$this->db->query($statements[6]);

		// Create Cart
		$this->db->query($statements[15]);

		// Customer Wishlist
		$this->db->query($statements[38]);

		$this->db->query("UPDATE `". DB_PREFIX ."country` SET `name` = 'South Korea' WHERE `". DB_PREFIX ."country`.`country_id` = 113");

		$this->db->query("UPDATE `". DB_PREFIX ."zone` SET `name` = 'Puducherry' WHERE `". DB_PREFIX ."zone`.`zone_id` = 1499;");

		$this->db->query("INSERT INTO `". DB_PREFIX ."zone` (`zone_id`, `country_id`, `name`, `code`, `status`) VALUES (NULL, '99', 'Telangana', 'TS', '1')");

		$this->db->query("INSERT INTO `". DB_PREFIX ."event` (`event_id`, `code`, `trigger`, `action`) VALUES (1, 'voucher', 'post.order.history.add', 'total/voucher/send')");

		// Update table Customer
		$this->db->query("ALTER TABLE `". DB_PREFIX ."_customer` CHANGE COLUMN `token` `token` text NOT NULL");

		// Drop customer_ban_ip
		$this->db->query("DROP TABLE IF EXISTS `". DB_PREFIX ."_customer_ban_ip`");

		// Modules
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "module`");

		foreach ($query->rows as $setting) {
			$value = unserialize($setting['setting']);
			$this->db->query("UPDATE `" . DB_PREFIX . "module` SET `setting` = '" . $this->db->escape(json_encode($value)) . "' WHERE `module_id` = '" . (int)$setting['module_id'] . "'");
		}
		...
		// Empty table customer_activity
		// $query = $this->db->query("TRUNCATE TABLE `" . DB_PREFIX . "customer_activity`");
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer_activity`");
		foreach ($query->rows as $result) {
			$value = unserialize($result['data']);
			$this->db->query("UPDATE `" . DB_PREFIX . "customer_activity` SET `data` = '" . $this->db->escape(json_encode($value)) . "' WHERE `activity_id` = '" . (int)$result['activity_id'] . "'");
		}

		// Affiliate Activity fix
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "affiliate_activity`");
		foreach ($query->rows as $result) {
			$value = unserialize($result['data']);
			$this->db->query("UPDATE `" . DB_PREFIX . "affiliate_activity` SET `data` = '" . $this->db->escape(json_encode($value)) . "' WHERE `affiliate_id` = '" . (int)$result['affiliate_id'] . "'");
		}
